...
#define SAMPLE_DEVICE_NAME "ESP_DEMO"
...
uint16_t BATT_heart_table[IDX_BATTERY_MAX];     // 电池电量服务句柄存储
...
static uint8_t raw_adv_data[] = {
    ...
    0x03, 0x03, 0xFF, 0x00,
    /* device name */
    0x0d, 0x09, 'R', 'B', 'P', '_', 'U', 'P', 'S', 'P', 'o', 'w', 'e','r'};
...
static const uint8_t BATT_value[1] = {0x63};    // BATT_value  = 0x00~0x64 = 0~100


/*电量服务UUID*/
static const uint16_t GATT_UUID_BATTERY_SVC = 0x180F;   // 电量服务的UUID
static const uint16_t GATT_UUID_BATTERY_CHAR = 0x2A19;  // 电量特征值
static const uint8_t char_prop_read_notify = ESP_GATT_CHAR_PROP_BIT_READ | ESP_GATT_CHAR_PROP_BIT_NOTIFY;


static const esp_gatts_attr_db_t gatt_db_BATTERY[IDX_BATTERY_MAX] =
    {
        // Service Declaration
        [IDX_SVC] =
        ...
        [IDX_CHAR_VAL_A] =
            {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATT_UUID_BATTERY_CHAR, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE, GATTS_DEMO_CHAR_VAL_LEN_MAX, sizeof(BATT_value), (uint8_t *)BATT_value}},
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_WRITE_EVT:
        ESP_LOGI("PowerState", "WRITE_EVT");
        // if (!param->write.is_prep)
        // {
            // the data length of gattc write  must be less than GATTS_DEMO_CHAR_VAL_LEN_MAX.
            ESP_LOGI(GATTS_TABLE_TAG, "GATT_WRITE_EVT, handle = %d, value len = %d, value :", param->write.handle, param->write.len);
            esp_log_buffer_hex(GATTS_TABLE_TAG, param->write.value, param->write.len);
            ESP_LOGI("PowerState", "error writing");
            if (param->write.handle == 47 && param->write.len == 1)
            {
                gpio_set_level(PW1555_EN, 1);
                ESP_LOGI("PowerState", "Power on");
            }
            else if (param->write.handle == 47 && param->write.len == 2)
            {
                gpio_set_level(PW1555_EN, 0);
                ESP_LOGI(GATTS_TABLE_TAG, "Power off");
            }
    ...
    case ESP_GATTS_CREAT_ATTR_TAB_EVT:
    {
        ...
        else if (param->add_attr_tab.svc_inst_id == 0)
        {
            ...
            if(ret)
            {
                ESP_LOGE(GATTS_TABLE_TAG, "创建失败报错为%x", ret);
            }
            else
            {
                ESP_LOGI(GATTS_TABLE_TAG, "0 create attribute table successfully, the number handle = %d\n", param->add_attr_tab.num_handle);
            }
        }
        else if(param->add_attr_tab.svc_inst_id == 1)
        {
            memcpy(BATT_heart_table, param->add_attr_tab.handles, sizeof(BATT_heart_table));
            esp_err_t ret = esp_ble_gatts_start_service(BATT_heart_table[IDX_SVC_BATTERY]);
            if(ret)
            {
                ESP_LOGE(GATTS_TABLE_TAG, "创建失败报错为%x", ret);
            }
            ...
            {
                ESP_LOGI(GATTS_TABLE_TAG, "1 create attribute table successfully, the number handle = %d\n", param->add_attr_tab.num_handle);
            }
        }
        break;
    }
